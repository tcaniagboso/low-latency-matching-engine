...
namespace order_book {
    AskMap ListOrderBook::empty_asks_;
    BidMap ListOrderBook::empty_bids_;
    ...
    AskMap::iterator ListOrderBook::remove_asks_level(
            types::Symbol symbol,
            AskMap::iterator it
    ) {
    ...
    BidMap::iterator ListOrderBook::remove_bids_level(
            types::Symbol symbol,
            BidMap::iterator it
    ) {
    ...
    AskMap::iterator ListOrderBook::asks_begin(types::Symbol symbol) {
        auto it = asks_.find(symbol);
        if (it == asks_.end()) return empty_asks_.end();
        return it->second.begin();
    }

    AskMap::iterator ListOrderBook::asks_end(types::Symbol symbol) {
        auto it = asks_.find(symbol);
        if (it == asks_.end()) return empty_asks_.end();
        return it->second.end();
    }

    BidMap::iterator ListOrderBook::bids_begin(types::Symbol symbol) {
        auto it = bids_.find(symbol);
        if (it == bids_.end()) return empty_bids_.end();
        return it->second.begin();
    }

    BidMap::iterator ListOrderBook::bids_end(types::Symbol symbol) {
        auto it = bids_.find(symbol);
        if (it == bids_.end()) return empty_bids_.end();
        return it->second.end();
    }
